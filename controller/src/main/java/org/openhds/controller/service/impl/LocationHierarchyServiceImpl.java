...
public class LocationHierarchyServiceImpl implements LocationHierarchyService {
    ...
    public LocationHierarchyServiceImpl(GenericDao genericDao, EntityService entityService,
            Generator locationGenerator, Generator locationHierarchyGenerator) {
    ...
    public LocationHierarchy evaluateLocationHierarchy(LocationHierarchy entityItem) throws ConstraintViolations {
        ...
        // when creating a Location Hierarchy item, the Parent may or may not be
        // specified
        ...
        if (item == null) {
            ...
            if (!checkRootAssigned())
                ...
                throw new ConstraintViolations(
                        "The Location Hierarchy Root has already been set. To make a new one you must delete the old root.");
        }

        // if the Parent item is not the root, then we can determine the level
        // of the Hierarchy item from the Parent
        else
    ...
    /**
     * To delete a Location Hierarchy item, there must be no dependent
     * Locations. Deleting a Location Hierarchy is only a safe operation if no
    ...
    public LocationHierarchy deleteLocationHierarchy(LocationHierarchy entityItem) throws ConstraintViolations {
        ...
        // item is at the lowest level so we need to check for any dependent
        // locations
        ...
        // the item is not at the lowest level so we need to check for any
        // arbitrary location
        else {
            // determine if the entityItem is a parent of any other existing
            // hierarchy items
    ...
    public boolean checkValidLocationEntry(String locExtId) {
        ...
        List<LocationHierarchy> list = genericDao.findListByProperty(LocationHierarchy.class, "level",
                getLowestLevel(), false);
    ...
    /**
     * Checks to see if a Hierarchy item has been assigned the root as the
     * parent
     */
    ...
    /**
     * Checks if a level violation will occur when attempting to edit a Location
     * Hierarchy item. It's possible that the hierarchy item is being used as a
     * Location and the level to become violated. This will result in a Location
     * referring to an invalid Location Hierarchy item. If this happens, the
     * Location Hierarchy item cannot be modified.
     */
    ...
    /**
     * Returns the root of the Hierarchy. This is not returning the Hierarchy
     * item at the highest level
     */
    ...
    /**
     * Retrieves all Location extId's which contain the term provided. Used in
    ...
    /**
     * Retrieves all Location ids within the hierarchy that contain the term
     * provided. Used in performing autocomplete.
     */
    ...
    /**
     * Find the locationHierarchy item by id
     */
